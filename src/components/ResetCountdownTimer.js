...
function nextDate(dayIndex) {
  const timeNow = new Date(),
    timeNowUTC = Date.UTC(timeNow.getUTCFullYear(), timeNow.getUTCMonth(), timeNow.getUTCDate(), 10, 0, 0),
...
const ResetCountdownTimers = () => {
  const timeNow = new Date(),
    timeNowUTC = Date.UTC(timeNow.getUTCFullYear(), timeNow.getUTCMonth(), timeNow.getUTCDate(), timeNow.getUTCHours(), timeNow.getUTCMinutes(), timeNow.getUTCSeconds()),
    timeResetUTC = Date.UTC(timeNow.getUTCFullYear(), timeNow.getUTCMonth(), timeNow.getUTCDate(), 10, 0, 0),
  ...
  if (nextDailyReset < startTime) {
    nextDailyReset = new Date(nextDailyReset.setDate(startTime.getDate() + 1))
  }
